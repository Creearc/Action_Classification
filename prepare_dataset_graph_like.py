...
PATH = '../Action_Recognition_Research/datasets/ue4'
    ...
    for ind, action in enumerate(actions):
        if action in file_name:
            for key, value in file_data.items():
                ...
                array_4x4x3 = np.zeros((4, 4, 3))

                for i in range(len(placement_array)):
                    for j in range(len(placement_array[i])):
                        if placement_array[i][j] > 0:
                            array_4x4x3[i, j] = vector[placement_array[i][j] - 4]

                data.append([array_4x4x3, ind])
                print(array_4x4x3.shape)
